...
export class MEM {
  ...
  constructor(config) {
    if (!config) {
      config = {
        MEM_SHARD_1_INTERNAL_HOST: "zygote-mem-shard-1",
        MEM_SHARD_2_INTERNAL_HOST: "zygote-mem-shard-2",
        MEM_SHARD_3_INTERNAL_HOST: "zygote-mem-shard-3",
  ...
  async connect() {
    ...
    if (!process.env.REDIS_NATMAP_DISABLED) {
      ...
      const { stdout: getnetResult } = await execP(
        "docker run --rm --network mynet  ghcr.io/evgnomon/ark:main getent hosts zygote-mem-shard-1 zygote-mem-shard-2 zygote-mem-shard-3"
      );
